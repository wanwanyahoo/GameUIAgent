...
if [[ -z "${GAMEUIAGENT_E2E_ZIP_PATH:-}" ]]; then
  echo "GAMEUIAGENT_E2E_ZIP_PATH is required" >&2
  exit 66
fi
...
GAMEUIAGENT_E2E_RESULT_PATH="$RESULT_PATH" GAMEUIAGENT_E2E_ZIP_PATH="$GAMEUIAGENT_E2E_ZIP_PATH" "$UNITY_EDITOR_EXECUTABLE" \
  -batchmode \
  -quit \
  -nographics \
